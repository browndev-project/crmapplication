...
class _ProjectEditDialogState extends ConsumerState<ProjectEditDialog> {
  ...
  late TextEditingController _totalAreaValueController;
  late TextEditingController _latController;
  late TextEditingController _lngController;
  
  final _r2Service = R2Service();
  late TextEditingController _reraIdController;
  late TextEditingController _possessionDateController;
  ...
  void initState() {
    ...
    _developerNameController = TextEditingController(text: p.developerName);
    _descriptionController = TextEditingController(text: p.description);
    
    // Location
    _address1Controller = TextEditingController(text: p.location?.address1 ?? '');
    _address2Controller = TextEditingController(text: p.location?.address2 ?? '');
    _cityController = TextEditingController(text: p.location?.city ?? '');
    _pincodeController = TextEditingController(text: p.location?.pincode ?? '');
    _stateController = TextEditingController(text: p.location?.state ?? '');
    _countryController = TextEditingController(text: p.location?.country ?? '');
    _latController = TextEditingController(text: p.location?.lat?.toString() ?? '');
    _lngController = TextEditingController(text: p.location?.lng?.toString() ?? '');

    // Area
    _totalAreaValueController = TextEditingController(text: p.totalArea?.value.toString() ?? '');
    _selectedAreaUnit = _normalizeUnit(p.totalArea?.unit);
    
    _isActive = p.active ;
    final dynamic rawStatus = (p.status as dynamic);
    _selectedStatus = (rawStatus != null && rawStatus.toString().isNotEmpty)
        ? rawStatus.toString().toLowerCase().replaceAll(' ', '_').replaceAll('-', '_')
        : 'active';
    final dynamic rawCategory = (p.category as dynamic);
    _selectedCategory = (rawCategory != null && rawCategory.toString().isNotEmpty)
        ? rawCategory.toString()
        : 'Residential';

    // New Fields Pre-population
    _reraIdController = TextEditingController(text: p.reraId ?? '');
    _possessionDateController = TextEditingController(text: p.possessionDate ?? '');
    _brochureUrlController = TextEditingController(text: p.brochureUrl ?? '');
    final dynamic rawAmenities = (p.amenities as dynamic);
    _amenitiesController = TextEditingController(text: rawAmenities != null ? (rawAmenities as List).join(', ') : '');
    _paymentPlanController = TextEditingController(text: p.paymentPlan ?? '');
    _videoUrlController = TextEditingController();

    final dynamic rawImages = (p.images as dynamic);
    _images = rawImages != null ? List<String>.from(rawImages.map((e) => e.toString())) : [];
    final dynamic rawVideos = (p.videos as dynamic);
    _videos = rawVideos != null ? List<String>.from(rawVideos.map((e) => e.toString())) : [];
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await ref.read(propertyProvider.notifier).updateProject(widget.project.id, payload);
             ...
             child: Column(
               ...
               children: [
                 ...
                 Row(
                   ...
                   children: [
                     Expanded(child: Text("Edit Project: ${widget.project.name}", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Theme.of(context).textTheme.bodyLarge?.color))),
                     InkWell(
