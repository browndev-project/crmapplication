...
class SalesExecutivesScreen extends ConsumerStatefulWidget {
  const SalesExecutivesScreen({super.key});

  @override
  ConsumerState<SalesExecutivesScreen> createState() => _SalesExecutivesScreenState();
}

class _SalesExecutivesScreenState extends ConsumerState<SalesExecutivesScreen> {
  final TextEditingController _searchController = TextEditingController();
  final ScrollController _scrollController = ScrollController();
  final String _systemRole = 'sales_executive'; 
  ...
  Widget build(BuildContext context) {
    ...
    final canView = permissions.hasModule(PermissionModules.STAFF_BASE, userRole: userRole) &&
        permissions.hasPermission(
          PermissionModules.SALES_EXEC_VIEW,
          userRole: userRole,
        );
    ...
    if (!canView) {
      return const Scaffold(
        ...
        appBar: GlobalAppBar(title: 'Sales Executives'),
    ...
    return Scaffold(
      ...
      appBar: const GlobalAppBar(title: 'Sales Executives'),
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Sales Executives', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87)),
                        const SizedBox(height: 4),
                        Text('Manage sales executives in your organization.', style: TextStyle(color: Colors.grey[600], fontSize: 13)),
                      ],
                    ),
                     Row(
                       children: [
                         _buildTopAction(Icons.refresh, () => ref.read(staffProvider(_systemRole).notifier).refresh(), isDark),
                         if (ref.watch(permissionsProvider).hasPermission(PermissionModules.SALES_EXEC_CREATE, userRole: ref.watch(loginProvider).user?.systemRole))
                            ...[
                ...
                Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Executive List', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                ...
                Container(
                  ...
                  //     hintText: 'Search executives, IDs, names...',
                ...
                if (staffState.users.isEmpty && !staffState.isLoading)
                  const Padding(
                    ...
                    child: Center(child: Text('No executives found', style: TextStyle(color: Colors.grey))),
                  )
                else
                  ListView.builder(
                    shrinkWrap: true,
  ...
  Widget _buildCardMenu(dynamic user) {
    return PopupMenuButton<String>(
      ...
      itemBuilder: (context) => [
        if (ref.watch(permissionsProvider).hasPermission(PermissionModules.SALES_EXEC_VIEW, userRole: ref.watch(loginProvider).user?.systemRole))
          const PopupMenuItem(
            value: 'view',
            child: Row(
              children: [
        ...
        if (ref.watch(permissionsProvider).hasPermission(PermissionModules.SALES_EXEC_UPDATE, userRole: ref.watch(loginProvider).user?.systemRole))
          const PopupMenuItem(
            value: 'edit',
            child: Row(
              children: [
