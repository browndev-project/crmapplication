...
class SalesManagersScreen extends ConsumerStatefulWidget {
  const SalesManagersScreen({super.key});

  @override
  ConsumerState<SalesManagersScreen> createState() => _SalesManagersScreenState();
}

class _SalesManagersScreenState extends ConsumerState<SalesManagersScreen> {
  final TextEditingController _searchController = TextEditingController();
  final ScrollController _scrollController = ScrollController();
  final String _systemRole = 'sales_manager'; 
  ...
  Widget build(BuildContext context) {
    ...
    final canView = permissions.hasModule(PermissionModules.STAFF_BASE, userRole: userRole) &&
        permissions.hasPermission(
          PermissionModules.SALES_MANAGER_VIEW,
          userRole: userRole,
        );
    ...
    if (!canView) {
      return const Scaffold(
        ...
        appBar: GlobalAppBar(title: 'Sales Managers'),
        body: AccessDeniedWidget(
          sectionName: "Sales Managers",
          showAppBar: false,
        ),
      );
    }
    ...
    return Scaffold(
      ...
      appBar: const GlobalAppBar(title: 'Sales Managers'),
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Sales Managers', style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87)),
                        const SizedBox(height: 2),
                        Text('Manage managers in your organization.', style: TextStyle(color: Colors.grey[600], fontSize: 13)),
                      ],
                    ),
                     Row(
                       children: [
                         _buildTopAction(Icons.refresh, () => ref.read(staffProvider(_systemRole).notifier).refresh(), isDark),
                         if (ref.watch(permissionsProvider).hasPermission(PermissionModules.SALES_MANAGER_CREATE, userRole: ref.watch(loginProvider).user?.systemRole))
                            ...[
                ...
                Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Sales Manager List', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                ...
                Container(
                  ...
                  //     hintText: 'Search managers, email, names...',
                          ...
                          if (_searchController.text.isNotEmpty)
                            IconButton(
                              ...
                              onPressed: () {
                                ...
                                ref.read(staffProvider(_systemRole).notifier).setSearch('');
                                setState(() {});
                              },
                            ),
                          IconButton(
                            ...
                            onPressed: () async {
                              ...
                              if (text != null && text.isNotEmpty) {
                                ref.read(staffProvider(_systemRole).notifier).setSearch(text);
                                setState(() {});
                              }
                            },
                          ),
                ...
                if (staffState.users.isEmpty && !staffState.isLoading)
                  const Padding(
                    ...
                    child: Center(child: Text('No managers found', style: TextStyle(color: Colors.grey))),
                  )
                else
                  ListView.builder(
                    shrinkWrap: true,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildInfoItem(Icons.group_work_outlined, 'Group', user.groupName ?? '-')),
                if (_systemRole == 'sales_manager')
                  Expanded(child: _buildInfoItem(Icons.layers_outlined, 'Teams', '${user.teamsCount}')),
              ],
            ),
  ...
  Widget _buildCardMenu(dynamic user) {
    return PopupMenuButton<String>(
      ...
      itemBuilder: (context) => [
        if (ref.watch(permissionsProvider).hasPermission(PermissionModules.SALES_MANAGER_VIEW, userRole: ref.watch(loginProvider).user?.systemRole))
          const PopupMenuItem(
            value: 'view',
            child: Row(
              children: [
        ...
        if (ref.watch(permissionsProvider).hasPermission(PermissionModules.SALES_MANAGER_UPDATE, userRole: ref.watch(loginProvider).user?.systemRole))
          const PopupMenuItem(
            value: 'edit',
            child: Row(
              children: [
